...
create table helper_kids.helper_kid_parent (
    helper_kid_parent_id varchar(64) primary key not null,
    helper_kid_id        varchar(64)             not null,
    parent_user_id       varchar(64)             not null,
    foreign key (helper_kid_id) references helper_kids.helper_kid (helper_kid_id) match simple on update no action on delete no action,
    foreign key (parent_user_id) references public."user" (user_id) match simple on update no action on delete no action
);

create table helper_kids.exchange_item (
    exchange_item_id varchar(64) primary key not null, -- ID
...
create table helper_kids.help_item (
    help_item_id varchar(64) primary key not null, -- ID
...
create table helper_kids.help_point_earned_achievement (
    help_point_earned_achievement_id varchar(64) primary key     not null, -- ID
    helper_kid_id                    varchar(64)                 not null, -- お手伝いキッズID
...
create table helper_kids.help_point_earned_detail (
    help_point_earned_detail_id varchar(64) primary key not null, -- ID
    earned_achievement_id       varchar(64)             not null, -- 獲得実績ID
    help_item_id                varchar(64)             not null, -- お手伝い項目ID
    help_item_count             integer                 not null, -- お手伝い項目件数
    help_item_total_point       integer                 not null, -- お手伝い項目合計ポイント
    group_id                    varchar(64)             not null, -- グループID
    foreign key (earned_achievement_id) references helper_kids.help_point_earned_achievement (help_point_earned_achievement_id) match simple on update no action on delete no action,
    foreign key (group_id) references public."group" (group_id) match simple on update no action on delete no action,
    foreign key (help_item_id) references helper_kids.help_item (help_item_id) match simple on update no action on delete no action
);
...
create table helper_kids.help_point_exchanged_achievement (
    help_point_exchanged_achievement_id varchar(64) primary key     not null, -- ID
    helper_kid_id                       varchar(64)                 not null, -- お手伝いキッズID
    exchange_item_id                    varchar(64)                 not null, -- 交換品ID
    ...
    group_id                            varchar(64)                 not null, -- グループID
    foreign key (exchange_item_id) references helper_kids.exchange_item (exchange_item_id) match simple on update no action on delete no action,
    foreign key (group_id) references public."group" (group_id) match simple on update no action on delete no action,
    foreign key (helper_kid_id) references helper_kids.helper_kid (helper_kid_id) match simple on update no action on delete no action
);
...
create table helper_kids.helper_kid_point (
    helper_kid_id      varchar(64) primary key     not null,                           -- お手伝いキッズID
